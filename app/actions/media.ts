...
// Créer un post pour les réseaux sociaux (Calendrier)
export async function createSocialPost(formData: FormData) {
  const user = await getCurrentUser();
  if (!user) return;

  const content = formData.get("content") as string;
  const platform = formData.get("platform") as string; // YOUTUBE, INSTAGRAM, etc.
...
// Sauvegarder un script ou une idée (Studio)
